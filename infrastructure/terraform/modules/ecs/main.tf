# ECS Fargate Module for EduNexus
# Cost-optimized container orchestration for demo/staging environments
# Uses Fargate to avoid EKS control plane costs ($73/month savings)
...
resource "aws_ecs_cluster_capacity_providers" "main" {
  ...
  default_capacity_provider_strategy {
    base              = 1
    weight            = 100
    capacity_provider = "FARGATE_SPOT"  # Cost optimization: use spot for demo
  }
}
...
# ALB Listener (HTTPS) - only created if certificate is provided
...
resource "aws_cloudwatch_log_group" "web" {
  ...
  retention_in_days = 7  # Cost optimization for demo
...
# ECR Lifecycle Policy (keep only last 5 images for cost)
...
output "log_group_names" {
  ...
  value = {
    ...
    ml  = aws_cloudwatch_log_group.ml.name
  }
}
